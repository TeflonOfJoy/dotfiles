...
if not args.skip_scrape or args.only_scrape_metadata:
    ...
    if not args.only_scrape_metadata:
        # Explicitly capture the cover page (page 0) which might be already loaded 
        # after metadata scraping but before regular page navigation begins
        print('Capturing cover page (page 0)...')
        cover_found = False
        ...
        if base_url:
            ...
            # If we're not starting at page 0, make sure to reload the starting page
            if args.start_page > 0:
                load_book_page(args.start_page)
            
            cover_found = True
            print('Cover image URL found')
